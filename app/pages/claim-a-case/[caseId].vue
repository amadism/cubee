...
<template>
  <h1 v-if="error && !gettingData" class="text-center px-4 py-8 text-lg sm:text-xl">invalid link</h1>
  <p v-if="gettingData" class="text-center px-4 py-8">loading...</p>
  <h1 v-if="gettingDataError && !caseData" class="text-center px-4 py-8 text-lg sm:text-xl">You are not authorized to claim this case.</h1>
  <div v-if="!gettingDataError && !gettingData" class="mx-auto mt-8 sm:mt-16 max-w-6xl px-2 sm:px-4">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
      ...
      <Card class="bg-yellow-50 border-yellow-400 border">
        <CardHeader class="p-4 sm:p-6">
          <CardTitle class="text-lg sm:text-xl text-yellow-800">
            ...
            <span class="text-xs sm:text-sm font-normal text-yellow-700">Nutzungsbedingungen und Vereinbarung</span>
          </CardTitle>
          ...
          >
            <div v-if="showConfirm && !successOpen" class="space-y-4">
              <div class="bg-white p-3 sm:p-4 rounded-lg border border-yellow-200">
                <h4 class="font-semibold text-yellow-800 mb-2 text-sm sm:text-base">🇩🇪 German:</h4>
        ...
        <CardFooter v-if="!successOpen" class="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-end p-4 sm:p-6">
          <Button :disabled="loading || isClaimed" variant="secondary" class="w-full sm:w-auto" @click="rejectClaim">Reject</Button>
      ...
      <Card class="bg-blue-50 border-blue-400 border h-fit">
        <CardHeader class="p-4 sm:p-6">
          <CardTitle class="text-lg sm:text-xl text-blue-800 flex items-center gap-2">
            <span>📋</span>
            Case Information
          </CardTitle>
        ...
        <CardContent class="p-4 sm:p-6">
          <div v-if="caseData && Object.keys(caseData).length > 0" class="space-y-3 sm:space-y-4">
            <!-- Report Type -->
            <div class="bg-white p-3 sm:p-4 rounded-lg border border-blue-200">
              <div class="flex items-center gap-2 mb-2">
                <span class="text-base sm:text-lg">🔧</span>
                <h4 class="font-semibold text-blue-800 text-sm sm:text-base">Report Type / Art des Berichts</h4>
              </div>
              <p class="text-xs sm:text-sm text-gray-700 capitalize">{{ (caseData as any).report_type || 'N/A' }}</p>
            </div>
            ...
            <div class="bg-white p-3 sm:p-4 rounded-lg border border-blue-200">
              <div class="flex items-center gap-2 mb-2">
                <span class="text-base sm:text-lg">🚗</span>
                <h4 class="font-semibold text-blue-800 text-sm sm:text-base">Vehicle / Fahrzeug</h4>
              </div>
              <p class="text-xs sm:text-sm text-gray-700">{{ (caseData as any).vehicle_make_model || 'N/A' }}</p>
            </div>
    ...
    >
      <div v-if="message && !data?.success" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        <div class="bg-gradient-to-br from-red-50 to-rose-50 border-2 border-red-400 rounded-xl shadow-2xl p-4 sm:p-6 lg:p-8 max-w-xl w-full text-center">
          <!-- Error Icon -->
          <div class="mb-2">
            <div class="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 bg-red-100 rounded-full mb-4">
              <span class="text-3xl sm:text-4xl">😢</span>
            </div>
            <div class="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-red-200 rounded-full -ml-4 -mt-2">
              <span class="text-2xl sm:text-3xl">❌</span>
            </div>
          </div>
          <p class="text-sm sm:text-base text-red-800 mb-4">
            Sorry, This case is already claimed.
            <br>
            Sorry, Dieser Fall ist bereits beansprucht.
          </p>
          <Button variant="primary" class="w-full text-sm sm:text-base" @click="router.push('/')">Okay, got it!</Button>
        </div>
      </div>
    </Transition>
    ...
    >
      <div v-if="successOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        ...
        >
          <div v-if="successOpen" class="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-400 rounded-xl shadow-2xl p-4 sm:p-6 lg:p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div class="text-center">
              ...
              <div class="mb-2">
                ...
                <div class="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-green-200 rounded-full -ml-4 -mt-2">
                  <span class="text-2xl sm:text-3xl">✅</span>
                </div>
              </div>
              ...
              <div class="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                <!-- German Message -->
                <div class="bg-white p-3 sm:p-4 rounded-lg border border-green-200 shadow-sm">
                  <h3 class="font-semibold text-green-800 mb-2 flex items-center gap-2 text-sm sm:text-base">
                    🇩🇪 <span>Herzlichen Glückwunsch!</span>
                  </h3>
                  <p class="text-xs sm:text-sm text-gray-700 leading-relaxed text-left">
              ...
              <div class="bg-green-100 p-3 sm:p-4 rounded-lg border border-green-300 mb-4 sm:mb-6">
                <p class="text-xs sm:text-sm text-green-800 font-medium">
                  🎯 Case ID: <span class="font-mono bg-white px-2 py-1 rounded text-xs sm:text-sm">{{ caseId }}</span>
                </p>
              ...
              <div class="flex justify-center">
                <Button 
                  variant="primary" 
                  class="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-lg shadow-lg transform hover:scale-105 transition-all duration-200 text-sm sm:text-base" 
                  @click="goHome"
                >
                  🚀 Okay, got it!
                </Button>
