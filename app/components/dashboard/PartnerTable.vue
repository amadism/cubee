  ...
  <div class="overflow-x-auto rounded-md font-poppins">
    <div class="flex mb-4 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex items-center gap-3">
        ...
        class="w-full sm:w-80 border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-0 focus:border-yellow-500 focus:border-2"
      />
      </div>
      <Button @click="$emit('add')" class="bg-emerald-600 hover:bg-emerald-700 text-white">Add Partner</Button>
    </div>
    <table class="min-w-full text-left text-xs lg:text-sm border rounded-md">
      <thead class="bg-gray-100 text-black font-medium">
        <tr class="border-b">
          <th class="w-[5%] border-r p-2 border-gray-200">#</th>
          <th class="w-[15%] border-r p-2 border-gray-200">{{ internalView === 'Pending Requests' ? 'Requested' : 'Created' }}</th>
          <th class="w-[20%] border-r p-2 border-gray-200">Name</th>
          <th class="w-[20%] border-r p-2 border-gray-200">{{ internalView === 'Pending Requests' ? 'Email' : 'Notes' }}</th>
          <th class="w-[15%] border-r p-2 border-gray-200">Tel</th>
          <th class="w-[17%] border-r p-2 border-gray-200">Location</th>
          <th class="w-[8%] p-2">Actions</th>
        </tr>
      </thead>
      <tbody v-show="!loading">
        ...
        >
          ...
          <td class="w-[20%] border-r p-2 truncate">{{ item.name }}</td>
          <td class="w-[20%] border-r p-2 truncate" :class="internalView === 'Pending Requests' ? '' : 'bg-green-50 text-green-800 font-bold'">{{ (internalView === 'Pending Requests' ? (item.email || 'not set') : (item.note || 'not set')).substring(0, 12) }}</td>
          <td class="w-[15%] border-r p-2 truncate">{{ item.tel }}</td>
          <td class="w-[17%] border-r p-2 truncate">{{ item.location_name }}</td>
          <td class="w-[8%] p-2">
        ...
        <tr v-if="!filtered?.length">
          <td colspan="7" class="p-3 text-center text-muted-foreground">No data</td>
        </tr>
      </tbody>
    </table>
